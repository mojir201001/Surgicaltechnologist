...
db.serialize(function () {
    ...
    db.run(
        "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT,telegram_id TEXT UNIQUE,phone TEXT,username TEXT,first_name TEXT,last_name TEXT,join_date DATETIME DEFAULT CURRENT_TIMESTAMP,status TEXT DEFAULT 'active')"
    );


    db.run(
        "CREATE TABLE IF NOT EXISTS buttons (id INTEGER PRIMARY KEY AUTOINCREMENT, parent_id INTEGER DEFAULT 0, title TEXT NOT NULL, mode TEXT DEFAULT 'none', position INTEGER DEFAULT 0, visible INTEGER DEFAULT 1, locked INTEGER DEFAULT 0, command TEXT, link TEXT,content_order TEXT DEFAULT 'new' , description TEXT , created_at DATETIME DEFAULT CURRENT_TIMESTAMP , updated_at DATETIME , updated_by TEXT)"                                                                                                                           
        
    );


    db.run(
        "CREATE TABLE IF NOT EXISTS files (id INTEGER PRIMARY KEY AUTOINCREMENT, button_id INTEGER, file_id TEXT, file_type TEXT, caption TEXT, position INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME,updated_by TEXT)"                                                                        
    );
    ...
    db.run("CREATE TABLE IF NOT EXISTS user_phones (id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,phone TEXT,created_at DATETIME DEFAULT CURRENT_TIMESTAMP)"
